...
def test(num_tokens: int, hidden: int, num_experts: int, num_topk: int,
    ...
    # Randomly mask some positions 不支持用-1做mask
    # for i in range(10):
    ...
    for i in range((num_times % 2) + 1):
        ...
        packed_recv_x, packed_recv_count, handle, event, hook = \
            buffer.low_latency_dispatch(x, topk_idx, ori_num_tokens, num_experts,
...
def test_main():
    ...
    num_tokens, hidden, num_topk, num_experts = 128, 7168, 8, 16
