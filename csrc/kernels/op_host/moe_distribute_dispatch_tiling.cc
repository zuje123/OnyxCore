...
//#include "../../../tbe/impl/ascendc/moe_distribute_dispatch_v2/moe_distribute_dispatch_v2_tiling.h"
    ...
    const bool isScales, const uint32_t quantMode, const bool isActiveMask)
{
    ...
    if (isActiveMask) {
        ...
        OP_TILING_CHECK(xActiveMaskStorageShape->GetStorageShape().GetDimNum() != ONE_DIM,
            OP_LOGE(nodeName, "xActiveMask shape dim must be 1, but current dim num is %lu.",
        ...
        OP_TILING_CHECK(xActiveMaskDim0 != xDim0, OP_LOGE(nodeName,
            "The input of xActiveMask dim0 = %ld is not equal to x dim0 = %ld.", xActiveMaskDim0, xDim0),
            return false);
        OP_LOGD(nodeName, "xActiveMask dim0 = %ld.", xActiveMaskDim0);
    }
    ...
    const bool isScales, const uint32_t quantMode, const bool isActiveMask)
{
    ...
    if (isScales) {
        ...
        OP_TILING_CHECK(scalesDesc->GetDataType() != ge::DT_FLOAT,
            OP_LOGE(nodeName, "scales dataType is invalid, dataType should be float, but is"
            ), return false);
    }
    ...
    if (isActiveMask) {
        ...
        OP_TILING_CHECK(xActiveMaskDesc->GetDataType() != ge::DT_BOOL, OP_LOGE(nodeName,
            "xActiveMask dataType is invalid, dataType should be bool, but is ."
            ), return false);
    }
    ...
    OP_TILING_CHECK(expertTokenNumsDesc->GetDataType() != ge::DT_INT64,
        OP_LOGE(nodeName, "expertTokenNums dataType is invalid, dataType should be int64, but is ."
        ), return false);
    ...
    MoeDistributeDispatchTilingData &tilingData, uint32_t &localMoeExpertNum)
{
    ...
    // 校验globalBS
    auto attrs = context->GetAttrs();
    bool isActiveMask = tilingData.moeDistributeDispatchInfo.isActiveMask;
    OP_TILING_CHECK(attrs == nullptr, OP_LOGE(nodeName, "attrs is nullptr."), return ge::GRAPH_FAILED);
...
static ge::graphStatus MoeDistributeDispatchA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    tilingData->moeDistributeDispatchInfo.isActiveMask = isActiveMask;
