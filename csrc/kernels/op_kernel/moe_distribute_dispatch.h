    ...
    GM_ADDR workspaceGM, TPipe *pipe, const MoeDistributeDispatchTilingData *tilingData)
{
    ...
    sharedExpertNum_ = tilingData->moeDistributeDispatchInfo.sharedExpertNum;
    ...
    isInputActiveMaskFlag_ = tilingData->moeDistributeDispatchInfo.isActiveMask;
    ...
    winDataSizeOffset_ = dataState_ * (tilingData->moeDistributeDispatchInfo.totalWinSize / 2)
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::SplitToCore(
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::FillTriple(
...
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::SendToSharedExpert()
{
    ...
    for (uint32_t virtualTokenIndex = startTokenId; virtualTokenIndex < endTokenId; ++virtualTokenIndex) {
        ...
        if constexpr (DynamicQuant || StaticQuant) {
            ...
            FillTriple(xOutTensor_, tokenIndex, axisK_ + toSharedExpertIndex);
            ...
            FillTriple(xTmpTensor_, tokenIndex, axisK_ + toSharedExpertIndex);
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::CalTokenSendExpertCnt(
...
{
    uint32_t curSendCnt = activeMaskBsCnt_ * axisK_;
    uint32_t startTokenId, endTokenId, sendTokenNum; // 每个aiv发送时的起始rankid
    SplitToCore(curSendCnt, moeUsedAivNum_, startTokenId, endTokenId, sendTokenNum);
    if (startTokenId >= curSendCnt) {return;}
    GlobalTensor<ExpandXOutType> dstWinGMTensor;
    DataCopyPadExtParams<XType> copyPadExtParams{false, 0U, 0U, 0U};
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::ActiveMaskCalCnt()
{
...
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::SetStatus()
{
    ...
    for (uint32_t rankIndex = startExpertId_; rankIndex < endExpertId_; ++rankIndex) {
        ...
        if (rankIndex >= sharedExpertRankNum_) {
            dstRankId = ((rankIndex - sharedExpertRankNum_) / moeExpertNumPerRank_ + sharedExpertRankNum_);
            offset += ((rankIndex - sharedExpertRankNum_) % moeExpertNumPerRank_ * epWorldSize_ * stateOffset_);
        }
        ...
        DataCopy<int32_t>(rankGMTensor, statusTensor_[rankIndex * 8], 8UL);  // 8是数据大小，按32对齐拷贝
    }
    SyncFunc<AscendC::HardEvent::MTE3_S>();
}

template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::ReduceMaxInplace(const LocalTensor<float>& srcLocal,
    uint32_t count)
{
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::SyncCntOnCore(
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::BufferInit()
{
    tpipe_->Reset();
...
template <TemplateMC2TypeClass>
__aicore__ inline void MoeDistributeDispatch<TemplateMC2TypeFunc>::GetCumSum(LocalTensor<int32_t> &outLocal, uint32_t totalCount)
{
